...
- **Imagens por família/subfamília** — `scripts/sync-atualcard-family-images.mjs` (todas as categorias), `scripts/sync-atualcard-business-card-images.mjs` (só Cartões de Visita) e `scripts/sync-atualcard-banner-images.mjs` (só Banners e Lonas) mapeiam produto → imagem por palavra-chave no nome. Ao adicionar imagens novas em `public/produtos/catalogo-atualcard/`, atualizar o mapa de regras no script correspondente antes de rodar.
